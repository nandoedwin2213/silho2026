...
export async function ServiceCard({ service, categorySlug }: { service: Pick<Service, "name" | "slug" | "shortDescription" | "basePrice" | "webPrice" | "priceFrom" | "showPrice" | "discountEligible" | "isSurgical" | "requiresMedicalAssessment" | "requiresManualQuote" | "image">; categorySlug: string }) {
  ...
  const pricing = await getWebPricingFor(service);
  ...
  return <Card className="group overflow-hidden rounded-2xl border-border/70 p-0 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
    <div className="relative aspect-[4/3] overflow-hidden rounded-t-2xl">
      ...
      {pricing.discountPercent > 0 && !priceHidden && <span className="absolute right-4 top-4 rounded-full bg-gold px-3 py-1 text-[11px] font-semibold text-ink">-{pricing.discountPercent}% web</span>}
    </div>
    <CardContent className="p-6">
      ...
      <div className="mt-5">{priceHidden ? <p className="text-sm text-muted-foreground">Precio tras valoración</p> : <WebPrice base={Number(service.basePrice)} webPrice={service.webPrice == null ? null : Number(service.webPrice)} discountPercent={pricing.discountPercent} priceFrom={service.priceFrom} />}</div>
