...
export function TreatmentBookingForm({ service, locations }: Props) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/orders", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...data, serviceId: service.id, pointsRedeemed: 0, acceptTerms: form.get("acceptTerms") === "on", acceptDataConsent: form.get("acceptDataConsent") === "on", paymentMethod: data.paymentMethod ?? "TRANSFER" }) });
