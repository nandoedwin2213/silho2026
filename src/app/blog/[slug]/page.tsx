...
export default async function BlogPostPage({ params }: Props) {
  ...
  return <main className="mx-auto max-w-4xl px-6 py-16 lg:px-10 lg:py-24"><div className="relative aspect-[21/9] overflow-hidden rounded-3xl"><Image src={blogImage(post.category, post.coverImage)} alt={post.title} fill priority sizes="(max-width: 1024px) 100vw, 896px" className="object-cover" /></div><p className="mt-10 text-xs uppercase tracking-[0.2em] text-gold">{post.category}</p><h1 className="mt-4 font-heading text-5xl tracking-tight text-navy">{post.title}</h1><p className="mt-6 text-lg leading-8 text-muted-foreground">{post.excerpt}</p><article className="prose prose-slate mt-12 max-w-none prose-headings:font-heading prose-headings:text-navy prose-a:text-gold" dangerouslySetInnerHTML={{ __html: html }} /><MedicalDisclaimer className="mt-12 border-t pt-6" /></main>;
}
