...
export default async function CategoryPage({ params }: Props) {
  ...
  return <main className="mx-auto max-w-7xl px-6 py-10 lg:px-10 lg:py-16"><section className="relative min-h-[320px] overflow-hidden rounded-[2rem] bg-navy px-7 py-16 text-white md:px-12"><Image src={categoryImage(category.slug, category.image)} alt="" fill sizes="(max-width: 1280px) 100vw, 1280px" className="object-cover opacity-50" /><div className="absolute inset-0 bg-gradient-to-r from-navy via-navy/75 to-navy/20" /><div className="relative"><p className="text-xs uppercase tracking-[0.24em] text-gold-light">Categoría SILHO</p><h1 className="mt-5 font-heading text-5xl tracking-tight md:text-6xl">{category.name}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-white/75">{category.description ?? "Opciones para conversar durante tu valoración médica."}</p></div></section><div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{category.services.map((service) => <ServiceCard key={service.id} service={service} categorySlug={category.slug} />)}</div></main>;
}
