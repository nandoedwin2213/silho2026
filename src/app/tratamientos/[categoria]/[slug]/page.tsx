...
export default async function ServiceDetailPage({ params }: Props) {
  const service = await getService(params);
  if (!service) notFound();
  const [surgicalSetting, number] = await Promise.all([getSetting("SHOW_SURGICAL_PRICES", "false"), getSetting("WHATSAPP_NUMBER", "593989049001")]);
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(json) }} /><section className="bg-[#fafaf9] px-6 py-16 lg:px-10 lg:py-24"><div className="mx-auto max-w-7xl"><Link href={`/tratamientos/${service.category.slug}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground transition hover:text-navy"><ArrowLeft className="size-4" /> {service.category.name}</Link><div className="mt-10 grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-start"><div><p className="text-xs uppercase tracking-[0.22em] text-gold">{service.category.name}</p><h1 className="mt-4 font-heading text-5xl tracking-tight text-navy md:text-6xl">{service.name}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-muted-foreground">{service.description}</p><div className="mt-8 flex flex-wrap gap-2">{service.concerns.map(({ concern }) => <span key={concern.id} className="rounded-full bg-white px-3 py-1.5 text-xs text-muted-foreground shadow-sm">{concern.name}</span>)}</div><div className="mt-10"><ServiceCTA service={service} /></div><div className="mt-5"><WhatsAppButton number={number ?? "593989049001"} service={service.name} variant="inline" /></div></div><div><div className="relative aspect-[4/3] overflow-hidden rounded-3xl"><Image src={serviceImage(service, service.category.slug)} alt={service.name} fill priority sizes="(max-width: 1024px) 100vw, 45vw" className="object-cover" /></div><div className="mt-6"><PriceBlock service={service} showSurgicalPrices={surgicalSetting === "true"} /><div className="mt-5"><MedicalDisclaimer /></div></div></div></div></div></section>{related.length > 0 && <section className="mx-auto max-w-7xl px-6 py-20 lg:px-10"><SectionHeading eyebrow="También puedes explorar" title="Tratamientos relacionados" /><div className="mt-10 grid gap-5 md:grid-cols-3">{related.map((item) => <ServiceCard key={item.id} service={item} categorySlug={item.category.slug} />)}</div></section>}</main>;
}
