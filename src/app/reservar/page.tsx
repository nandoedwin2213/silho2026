...
export default async function BookingPage({ searchParams }: { searchParams: Promise<{ objetivo?: string; ciudad?: string }> }) {
  ...
  const service = await db.service.findFirst({ where: { slug: settings.ASSESSMENT_SERVICE_SLUG ?? "valoracion-valoracion-estetica-facial", active: true }, select: { id: true, slug: true, name: true, basePrice: true, webPrice: true, discountEligible: true } });
